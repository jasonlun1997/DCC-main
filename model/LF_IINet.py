...
class get_model(nn.Module):
    ...
    def forward(self, x, info=None):
        ...
        intra_fea_initial = self.IntraFeaExtract(x_multi)   #13    
        inter_fea_initial = self.InterFeaExtract(x_multi)   #13
        ...
        intra_fea_0, inter_fea_1 = self.Interact_1(intra_fea_initial, inter_fea_initial) #7
        intra_fea_0 = self.MCB_1(intra_fea_0)   #6
        
        intra_fea_1, inter_fea_2 = self.Interact_2(intra_fea_0.permute(0,2,1,3,4), inter_fea_1)#7
        intra_fea_1 = self.MCB_2(intra_fea_1)   #6
        
        intra_fea_2, inter_fea_3 = self.Interact_3(intra_fea_1.permute(0,2,1,3,4), inter_fea_2)#7
        intra_fea_2 = self.MCB_3(intra_fea_2)   #6
        
        intra_fea_3, _ = self.Interact_4(intra_fea_2.permute(0,2,1,3,4), inter_fea_3)#5
        intra_fea_3 = self.MCB_4(intra_fea_3)   #6
        
        intra_fea = torch.cat((intra_fea_0, intra_fea_1, intra_fea_2, intra_fea_3), 1).permute(0,2,1,3,4)

        intra_fea = self.FBM(intra_fea)     #32
        out_sv = self.UpSample(intra_fea)   #3
...
class FeaExtract(nn.Module):#13
    def __init__(self, channel):
        super(FeaExtract, self).__init__()
        self.FEconv = nn.Conv2d(1, channel, kernel_size=1, stride=1, padding=0, bias=False)#1
        self.FERB_1 = ResASPP(channel)#4
        self.FERB_2 = RB(channel)#2
        self.FERB_3 = ResASPP(channel)#4
        self.FERB_4 = RB(channel)#2
...
class Extract_inter_fea(nn.Module):#13
    def __init__(self, channel, angRes):
        super(Extract_inter_fea, self).__init__()
        self.FEconv = nn.Conv2d(angRes*angRes, channel, kernel_size=1, stride=1, padding=0, bias=False)#1
        self.FERB_1 = ResASPP(channel)#4
        self.FERB_2 = RB(channel)#2
        self.FERB_3 = ResASPP(channel)#4
        self.FERB_4 = RB(channel)#2
