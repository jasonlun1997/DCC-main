...
from model.SR.dcn.modules.deform_conv import DeformConv

class get_model(nn.Module):#Net
    def __init__(self, args):#angRes, factor
        super(get_model, self).__init__()
        n_blocks, channel = 4, 32
        self.factor = args.scale_factor#factor
        self.angRes = args.angRes_in#angRes
        self.FeaExtract = FeaExtract(channel)
        self.ADAM_1 = ADAM(channel, args.angRes_in)#angRes
        self.ADAM_2 = ADAM(channel, args.angRes_in)#angRes
        self.ADAM_3 = ADAM(channel, args.angRes_in)#angRes
        self.Reconstruct = CascadedBlocks(n_blocks, 4*channel)
        self.UpSample = Upsample(channel, args.scale_factor)#factor
...
class ADAM(nn.Module):
    def __init__(self, channel, angRes):
        ...
        #
        # offset = torch.Tensor(np.zeros[18,3,3,3])
        # weight = torch.Tensor(np.zeros[18,3,3,3])
        # self.dcn = deform_conv2d(channel, offset, weight, stride=1, padding=1)#add
